<?php
  ...
  // $sql= mysqli_query($conn, "UPDATE artesvisuais_1a SET primeiroTrim=$value");
    $curryear = date("Y");
    $sql = "SELECT CONCAT(profiles.first_name, ' ', profiles.last_name) as fullname, disciplina.nome_id, disciplina.prim_trim, disciplina.seg_trim, disciplina.ter_trim, disciplina.notafinal FROM $disc as disciplina, profiles WHERE disciplina.nome_id=profiles.id AND disciplina.year=$curryear ORDER by fullname";
