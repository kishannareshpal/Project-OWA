  <!DOCTYPE html>
  <html lang="pt_PT">
  <head>
    <!-- TODO: Add essential meta tags later -->
    <meta charset="UTF-8">
    <meta author="www.oneworldacademymz.com">
    <meta co-author="Kishan Nareshpal Jadav">
    <title>Our Team - OWA</title>
    <meta http-equiv="X-UA-Compatible" content="IE=edge">
    <meta name="description" content="One World Academy">
    <meta name="viewport" content="width=device-width, initial-scale=1, shrink-to-fit=no">

    <!-- Add to homescreen for Chrome on Android -->
    <meta name="mobile-web-app-capable" content="yes">
    <link rel="icon" sizes="192x192" href="../images/android-desktop.png">

    <!-- Add to homescreen for Safari on iOS -->
    <meta name="apple-mobile-web-app-capable" content="yes">
    <!-- Depprecated -->
    <!-- <meta name="apple-mobile-web-app-status-bar-style" content="black"> -->
    <meta name="apple-mobile-web-app-title" content="OWA">
    <link rel="apple-touch-icon-precomposed" href="../images/ios-desktop.png">

    <link rel="shortcut icon" href="../images/favicon.png">

    <!-- hosted css's -->
    <link rel="stylesheet" href="https://fonts.googleapis.com/icon?family=Material+Icons">
    <link rel="stylesheet" type="text/css" href="https://cdnjs.cloudflare.com/ajax/libs/semantic-ui/2.3.0/semantic.min.css">
    <link rel="stylesheet" href="../required/js/material-components-web/dist/material-components-web.min.css">
    <link rel="stylesheet" href="https://code.getmdl.io/1.3.0/material.teal-blue.min.css">
    <link rel="stylesheet" href="https://fonts.googleapis.com/css?family=Cinzel+Decorative|Open+Sans|Karma">
    <link rel="stylesheet" href="https://www.w3schools.com/w3css/4/w3.css">
    <link rel="stylesheet" href="../required/css/app.css">
    <style media="screen">
      *{
        font-family: 'Karma';
      }
    </style>
    <script src="//code.jquery.com/jquery-latest.min.js"></script>
  </head>
  <body class="mdl-color--grey-100">

    <!-- Page Body -->
    <div class="mdl-layout mdl-js-layout mdl-layout--fixed-header">
      <header style="height: 150px; background-color: rgba(180, 236, 192, 0.63)" class="mdl-layout__header mdl-layout__header--scroll mdl-layout__header--transparent"> <!-- himage -->
        <!-- page header -->
        <div style="height: 150px; padding-left: 5px" class="mdl-layout__header-row mdl-shadow--4dp">
          <div class="mdl-layout-spacer"></div>
          <!-- Title -->
          <span><img src="../images/logo.png" width='60px'></span>
          <a href="../" style="text-decoration: none; color: inherit;; color: inherit;" class="mdl-layout-title">
						<h3 style="text-decoration: none; text-decoration: none; margin-bottom: 0px; font-family: 'Karma'; font-size: 14px; margin-bottom: 0px; text-shadow: 1px 1px rgba(43, 43, 43, 0.8); margin-top: 0px" class="mdl-layout-title"><strong>Escola Primária</strong></h3>
            <span style="margin-bottom: 0px; margin-top: 0px; font-family: 'Karma'; font-size: 18px; text-shadow: 1px 1px rgba(43, 43, 43, 0.8);">
              <strong>One World Academy</strong>
            </span>
            <h5 style="font-family: 'Karma'; font-size: 14px; margin-bottom: 0px; margin-top: 0px" class="mdl-layout-title mdl-color-text--grey-700"><strong>Maxixe, Mozambique</strong></h5>
          </a>
          <!-- Add spacer, to align navigation to the right -->
          <div class="mdl-layout-spacer"></div>
        </div>
      </header>

      <main class="mdl-layout__content">
        <!-- page content -->
        <div class="mdl-grid" style="margin-bottom: 350px;">
          <div class="mdl-layout-spacer"></div>
            <div style="margin-top: 30px" class="mdl-cell mdl-cell--10-col">

              <div class="mdl-grid" style="margin-bottom: 10px;"> <!-- position: relative; bottom: 17px -->
                <!-- <div class="mdl-layout-spacer"></div> -->
                <div class="mdl-layout-spacer"></div>

                <a target="_blank" href="https://www.facebook.com/OWAmaxixe" class="ui small facebook">
                  <i class="facebook mdl-color-text--blue-800 icon large"></i>
                </a>
                <a target="_blank" href="https://www.instagram.com/oneworldmz" class="ui small instagram">
                  <i class="instagram insta-outline icon large"></i>
                </a>
                <a target="_blank" href="https://www.twitter.com/oneworldmz" class="ui small twitter">
                  <i class="twitter mdl-color-text--blue-300 icon large"></i>
                </a>
                <a target="_blank" href="https://www.youtube.com/" class="ui small youtube">
                  <i class="youtube mdl-color-text--red-800 icon large"></i>
                </a>
              </div>

              <nav class="mdl-navigation mdl-shadow--2dp mdl-color--grey-50" style="padding: 20px; margin-bottom: 30px; border-radius: 5px">
                <div class="mdl-grid">
                  <a href='../              ' style="color: grey ; margin: 5px; padding-top: 2px; border-radius: 7px; font-family: karma"    class="mdl-button mdl-js-button mdl-color--green-50">Home</a>
                  <a href='../about/        ' style="color: grey; margin: 5px; padding-top: 2px; border-radius: 7px; font-family: karma"     class="mdl-button mdl-js-button mdl-color--green-50">Sobre nós</a>
                  <a href='#                ' style="color: white; margin: 5px; padding-top: 2px; border-radius: 7px; font-family: karma"    class="mdl-button mdl-js-button mdl-color--green-500">A Equipe</a>
                  <a href='                 ' style="color: grey ; margin: 5px; padding-top: 2px; border-radius: 7px; font-family: karma"    class="mdl-button mdl-js-button mdl-color--green-50">Actividades</a>
                  <a href='../getinvolved/  ' style="color: grey ; margin: 5px; padding-top: 2px; border-radius: 7px; font-family: karma"    class="mdl-button mdl-js-button mdl-color--green-50">Envolver-se</a>
                  <a href='../contacts/     ' style="color: grey ; margin: 5px; padding-top: 2px; border-radius: 7px; font-family: karma"    class="mdl-button mdl-js-button mdl-color--green-50">Contactos</a>
                  <a href='../alunos/login/    ' style="color: grey ; margin: 5px; padding-top: 2px; border-radius: 7px; font-family: karma" class="mdl-button mdl-js-button mdl-color--orange-50"><i>Meu Aluno</i></a>
                </div>
              </nav>

              <h1 style="font-size: 40px; padding-left: 5px; text-shadow: 3px 3px rgba(180, 236, 192, 0.63);"><strong>Conheça a Equipe</strong></h1>
              <!-- <h1 style="margin-top: 5px; font-size: 28px; padding: 10px; padding-left: 20px; text-shadow: 3px 3px rgba(124, 145, 128, 0.63); border-radius: 5px 50px 5px 5px" class="bg-grad mdl-color-text--grey-700"><strong>_About Us</strong></h1> -->
              <!-- OUR VISION -->
              <div style="width: 100%; border-radius: 10px" class="mdl-card">
                <div style="width: 100%" class="mdl-card__supporting-text">
                  <h3 style="position: relative; left: 10px" class="mdl-color-text--green-500"><strong>Os nossos professores</strong></h3>

                  <!-- actual page content -->
                  <div class="mdl-grid">
                    <div class="mdl-layout-spacer"></div>

                      <div class="ui four stackable cards">
                        <div class="yellow mdl-color--grey-50 card">
                          <div class="image">
                            <img src="profpic/roque.jpg">
                          </div>
                          <div class="content">
                            <div class="header">Roque Diogo da Fonseca Malandela</div>
                            <div class="meta">
                              Desde 2017
                            </div>
                          </div>
                          <div class="extra content mdl-color-text--grey-700">
                            <span style="font-size: 14px;">
                              Diretor/Professor da 4ª Classe
                            </span>
                          </div>
                        </div>

                        <div class="black mdl-color--grey-50 card">
                          <div class="image">
                            <img src="profpic/rita.jpg">
                          </div>
                          <div class="content">
                            <div class="header">Rita Macane Tomberli</div>
                            <div class="meta">
                              Desde 2015
                            </div>
                          </div>
                          <div class="extra content mdl-color-text--grey-700">
                            <span style="font-size: 14px">
                              Diretora Assistente/Professora da Pré
                            </span>
                          </div>
                        </div>

                        <div class="red mdl-color--grey-50 card">
                          <div class="image">
                            <img src="profpic/irene.jpg">
                          </div>
                          <div class="content">
                            <div class="header">Irene Anatásio Sinai</div>
                            <div class="meta">
                              Desde 2015
                            </div>
                          </div>
                          <div class="extra content mdl-color-text--grey-700">
                            <span style="font-size: 14px">
                              Professora da 1ª Classe
                            </span>
                          </div>
                        </div>

                        <div class="blue mdl-color--grey-50 card">
                          <div class="image">
                            <img src="profpic/moises.jpg">
                          </div>
                          <div class="content">
                            <div class="header">Moisés da Silva Samuel</div>
                            <div class="meta">
                              Desde 2018
                            </div>
                          </div>
                          <div class="extra content mdl-color-text--grey-700">
                            <span style="font-size: 14px">
                              Professor da 2ª Classe
                            </span>
                          </div>
                        </div>

                        <div class="teal mdl-color--grey-50 card">
                          <div class="image">
                            <img src="profpic/amelia.jpg">
                          </div>
                          <div class="content">
                            <div class="header">Amélia Moreira de Jesús Teresa</div>
                            <div class="meta">
                              Desde 2017
                            </div>
                          </div>
                          <div class="extra content mdl-color-text--grey-700">
                            <span style="font-size: 14px">
                              Professora da 3ª Classe
                            </span>
                          </div>
                        </div>

                        <div class="green mdl-color--grey-50 card">
                          <div class="image">
                            <img src="profpic/teodosio.jpg">
                          </div>
                          <div class="content">
                            <div class="header">Teodósio Paulino Mutambe</div>
                            <div class="meta">
                              Desde 2017
                            </div>
                          </div>
                          <div class="extra content mdl-color-text--grey-700">
                            <span style="font-size: 14px">
                              Professor da Ed. Física
                            </span>
                          </div>
                        </div>

                        <div class="purple mdl-color--grey-50 card">
                          <div class="image">
                            <img src="profpic/eric.jpg">
                          </div>
                          <div class="content">
                            <div class="header">Eric Christian Cooper</div>
                            <div class="meta">
                              Desde 2015
                            </div>
                          </div>
                          <div class="extra content mdl-color-text--grey-700">
                            <span style="font-size: 14px">
                               Professor de Inglês para a Pré, 1ª e para a 2ª Classe.
                            </span>
                          </div>
                        </div>

                        <div class="black mdl-color--grey-50 card">
                          <div class="image">
                            <img src="profpic/silvino.jpg">
                          </div>
                          <div class="content">
                            <div class="header">Silvino Augusto Lambo</div>
                            <div class="meta">
                              Desde 2018
                            </div>
                          </div>
                          <div class="extra content mdl-color-text--grey-700">
                            <span style="font-size: 14px">
                               Professor de Música
                            </span>
                          </div>
                        </div>

                        <div class="blue mdl-color--grey-50 card">
                          <div class="image">
                            <img src="profpic/alisa.jpg">
                          </div>
                          <div class="content">
                            <div class="header">Alisa Cacho-Sousa</div>
                            <div class="meta">
                              Desde 2017
                            </div>
                          </div>
                          <div class="extra content mdl-color-text--grey-700">
                            <span style="font-size: 14px">
                               Professora de Inglês para a 3ª e 4ª Classe
                            </span>
                          </div>
                        </div>

                        <div class="yellow mdl-color--grey-50 card">
                          <div class="image">
                            <img src="profpic/agostinho.jpg">
                          </div>
                          <div class="content">
                            <div class="header">Agostinho Álvaro Marengula Matsombe</div>
                            <div class="meta">
                              Desde 2015
                            </div>
                          </div>
                          <div class="extra content mdl-color-text--grey-700">
                            <span style="font-size: 14px">
                               Professor de Artes
                            </span>
                          </div>
                        </div>

                        <div class="brown mdl-color--grey-50 card">
                          <div class="image">
                            <img src="profpic/zito.jpg">
                          </div>
                          <div class="content">
                            <div class="header">Zito Jorge Ngovene</div>
                            <div class="meta">
                              Desde 2016
                            </div>
                          </div>
                          <div class="extra content mdl-color-text--grey-700">
                            <span style="font-size: 14px">
                             Professor de Dança
                            </span>
                          </div>
                        </div>

                        <div class="brown mdl-color--grey-50 card">
                          <div class="image">
                            <img src="profpic/elisa.jpg">
                          </div>
                          <div class="content">
                            <div class="header">Elisa Albano Massingue</div>
                            <div class="meta">
                              <span class="date">
                                Desde 2017
                              </span>
                            </div>
                          </div>
                          <div class="extra content mdl-color-text--grey-700">
                            <span style="font-size: 14px">
                               Assistente da Secretária/Pré
                            </span>
                          </div>
                        </div>

                      </div>

                    </div>

                  <!-- end of actual page content -->
                </div>
              </div>

            </div>

          <div class="mdl-layout-spacer"></div>
        </div>
        <!-- end of page content -->


        <footer id="footer" hidden style="text-align: center; font-family: karma" class="mdl-mega-footer mdl-color--grey-50">
          <div class="mdl-mega-footer__middle-section">
          </div>

          <div class="mdl-mega-footer__bottom-section">
            <div class="mdl-layout-spacer"></div>
            <div class="mdl-grid" style="padding: 0px; margin-bottom: 22px;"> <!-- position: relative; bottom: 17px -->
              <div class="mdl-layout-spacer"></div>
              <a href="https://www.oneworldacademymz.com/ourteam/" class="mdl-button mdl-js-button mdl-color--grey-200" style="text-transform: none; font-family: 'Karma'; color: grey; padding-top: 2px; border-radius: 3px" tabindex="0">
                <i class="united states flag"></i>
                View the site in English
              </a>
            </div>
            <img src="../images/logo.png" width='32px' alt="">
            <p style="margin-bottom: 0px; cursor: none; font-size: 14px">1 Bairro Eduardo Mondlane</p>
            <p style="margin-top: 0px; margin-bottom: 0px; font-size: 14px">Maxixe, Mozambique</p>
            <!-- <p style="margin-top: 0px">Phone: </p> -->
            <p style="margin-top: 0px; margin-bottom: 0px; font-size: 14px">Celular: <a href="tel:+258843549804">+258 84-354-9804</a></p>
            <p style="margin-top: 0px; font-size: 14px">Email: <a target="_blank" href="mailto:oneworldacademymz@gmail.com">oneworldacademymz@gmail.com</a></a>
            <h6 style="font-size: 14px; font-family: karma"><strong>Copyright © <script>document.write((new Date()).getFullYear())</script> <span class="mdl-color-text--green-300">One World Academy Primary School</span></strong></h6>

            <!-- <ul class="mdl-mega-footer__link-list">
              <li><a href="#">About</a></li>
              <li><a href="#">Terms</a></li>
              <li><a href="#">Partners</a></li>
              <li><a href="#">Updates</a></li>
            </ul> -->
          <div class="mdl-layout-spacer"></div>

            <!-- <div><script>document.write((new Date()).getFullYear())</script> © One World Academy Elementary School</div>
            <p>Maxixe, Mozambique</p>
            <div>All rights reserved.</div>
            <p class="mdl-color-text--red-400">Notice: This web app is under development. Build_nr. A<script>document.write(localStorage.getItem("app_version_owa"))</script></p>
            <div class="ui buttons">
              <button class="ui disabled button">Change to</button>
              <button class="ui icon button">
                Portuguese
              </button>
            </div> -->
          </div>
        </footer>
      </main>
    </div>

    <!-- Imports -->
    <script defer src="https://code.getmdl.io/1.3.0/material.min.js"></script>
    <script defer src="../required/js/material-components-web/dist/material-components-web.min.js"></script>
    <script defer src="https://cdnjs.cloudflare.com/ajax/libs/semantic-ui/2.3.0/semantic.min.js"></script>
    <!-- <script defer src="../required/js/siema.min.js"></script> -->
    <script defer src="../required/js/app.js"></script>


  </body>
  </html>
